...
def _copy_input_snapshot_from_prev(prev_run_dir: Path, cur_run_dir: Path) -> None:
    """
    Create cur_run_dir/input_snapshot/ by copying key result CSVs from
    prev_run_dir/output/. This enables stable BEFORE/AFTER graphing:
      BEFORE = cur_run_dir/input_snapshot/*.csv
      AFTER  = cur_run_dir/output/*.csv
    """
    ...
    snap_dir = cur_run_dir / "input_snapshot"
    snap_dir.mkdir(parents=True, exist_ok=True)

    # minimum set for graphing
    candidates = [
        ("one_node_result_timeseries.csv", "one_node_result_timeseries.csv"),
        ("money_timeseries.csv", "money_timeseries.csv"),
    ]
    copied = 0
    for src_name, dst_name in candidates:
        src = prev_out / src_name
        dst = snap_dir / dst_name
        if src.exists() and _safe_copy(src, dst):
            copied += 1

    # optional: also keep the "before" overview image if you want
    # (handy for eyeballing)
    # for p in prev_out.glob("psi_overview__*.png"):
    #     _safe_copy(p, snap_dir / p.name)

    print(f"[queue] input_snapshot copied={copied} from prev={prev_run_dir.name} -> cur={cur_run_dir.name}")
